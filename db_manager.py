...
word = str(input("Enter the name you wanna hear: "))

conn = mysql.connector("sample.db")
cur = conn.cursor()


def create_table():
    cur.execute("CREATE TABLE test (Char VARCHAR, Sound VARCHAR)")
    
    
def entering_data():
    cur.execute("INSERT INTO test VALUES('أ', 'rC:\Users\Muhammad\Desktop\ArabicAlphabet\أ.mp3')")
    cur.execute("INSERT INTO test VALUEs('ب', 'rC:\Users\Muhammad\Desktop\ArabicAlphabet\ب.mp3')")
    conn.commit()

listo = []
def read_data(i):
for i in word:
    sql = 'SELECT * FROM test WHERE Char = ? '
